...
(z = |theta_val - median| / MAD, compared against mad_threshold) is governed
by the mad_threshold keyword argument, so callers can control what counts as
an artifact.
...
def _run(theta_values, **loop_kwargs):
    ...
    so it's read from the return value rather than main_pipeline module
    state).
    """
