...
the other threshold constants) as keyword arguments, defaulting to the
...
cooldown_time and a small max_sonications to exercise the real cap logic in
a fraction of a second, instead of needing 10 * (5s sonication + 15s
cooldown) of real wall-clock time.
...
def _run(theta_values, **loop_kwargs):
    ...
    local to theta_trigger_loop, so it's read from the return value rather
    than main_pipeline module state).
    """
