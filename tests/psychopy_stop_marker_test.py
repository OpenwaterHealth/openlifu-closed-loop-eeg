...
The round trip has two independent halves, both checked here:

1. The producer side: N-back_lastrun.py's source must push a
   "STOP_EXPERIMENT" marker when it detects the -1.0 stop sentinel from
   main_pipeline.py's stop_psychopy(), not just update its own local
   PsychoPy state. This is a source-text check rather than a live run,
   since driving the real PsychoPy script needs an actual visual
   window/keyboard.

2. The consumer side: main_pipeline.py's listen_for_start_stop() must flip
   psychopy_running back to False when a "STOP_EXPERIMENT" marker arrives
   over the real "PsychoPyMarkers" LSL stream. This side is covered here
   because the other tests in this suite (theta_lifu_validation_test.py,
   num_sonications_test.py, mad_threshold_test.py) deliberately bypass
   listen_for_start_stop() and set main_pipeline.psychopy_running directly,
   mirroring its effect without any LSL -- so this file is what actually
   exercises the real listener end-to-end.
...
SRC_DIR = REPO_ROOT / "src" / "openlifu_closed_loop"
...
def test_lastrun_script_pushes_stop_experiment_on_sentinel():
    ...
    -- not just set the status locally. This is a source-text check rather
    than a live PsychoPy run, since that needs a real window/keyboard.
    """
    ...
    assert occurrences >= 2, (
        ...
        f"{occurrences} -- the -1.0 sentinel handling must push "
        "STOP_EXPERIMENT, not just set thisExp.status = FINISHED, so "
        "main_pipeline.py learns the experiment stopped"
    )
...
def test_stop_experiment_marker_resets_psychopy_running(recorded_events):
    ...
    try:
        ...
        assert _push_until(
            ...
            "marker -- without N-back_lastrun.py pushing this marker, "
            "listen_for_start_stop() never hears that the experiment stopped"
        )
...
def test_theta_crossing_after_stop_experiment_does_not_sonicate(recorded_events):
    ...
    it -- a stale in-flight theta sample must not sonicate someone after the
    experiment has already ended.

    Sequenced as one lifecycle rather than two separate tests so the second
    half is a meaningful check and not a tautology: the same kind of in-band
    sample is shown to actually sonicate *before* the stop, and then shown
    to be suppressed *after* it, against the real listen_for_start_stop()
    listener (not a hand-set flag).
    """
    events = recorded_events
    ...
    try:
        ...
        # 4. Stop the experiment for real, over LSL -- the gate under test.
        assert _push_until(
            marker_outlet, ["STOP_EXPERIMENT"], lambda: main_pipeline.psychopy_running is False
